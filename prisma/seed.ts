...
const seedIdeas: SeedIdea[] = [
  {
    ...
    mockupPrompt:
      "UI mockup screenshot, Samsung AU Galaxy S26 Ultra product buy page on mobile (393×852). Sticky hubble-price-bar at bottom spanning full width, 56px height, dark background (#1A1A2E) with 1px top border #333. Left: total price '$1,949 AUD' in 20px white Samsung One bold, 'incl. trade-in' label in 10px #999 below. Center: orange (#FF6B35) low-stock badge pill with white 11px text 'Only 4 left — Selling fast' and stock icon. Badge has 2px rounded corners and orange-tinted background (#FFF3ED). Right: blue 'Add to Cart' button (#2189FF) with 24px rounded corners, 48px height, 14px white bold text. Product page content (device image, color selector, key specs) partially visible above the sticky bar. Clean Samsung One UI style.",
  },
  {
    ...
    mockupPrompt:
      "UI mockup screenshot, Samsung AU Galaxy S26 Ultra product buy page on mobile (393×852). Above the sticky hubble-price-bar, a slim horizontal notification badge (40px height, full width minus 16px margins, 6px rounded corners) with light green (#E8F5E9) background and 1px #C8E6C9 border. Left: eye icon 👁 (16px) in #4CAF50. Text: '23 people are viewing this phone right now' in 13px #2E7D32 Samsung One. Below badge, standard hubble-price-bar: price $1,949 in white bold, delivery info in 11px #AAA, blue 'Add to Cart' button (#2189FF) right-aligned. Phone image gallery with swipe dots and color option pills partially visible above. Clean Samsung design with consistent 16px horizontal margins.",
  ...
  },
  {
    ...
    categorySlug: "ux",
    mockupPrompt:
      "UI mockup screenshot, Samsung AU product buy page on mobile (393×852). Floating circular 'Compare' button (56px diameter, white, 4px shadow, compare icon in #333) positioned bottom-right, 16px from right edge and 80px from bottom. Comparison panel covering bottom 60% of screen with white background, 12px rounded top corners, and drag handle bar (32px, #DDD) at top center. Panel shows comparison table with 14px header 'Compare Models': Galaxy S26 Ultra vs Galaxy S26+ vs Galaxy S25 Ultra in 3 columns. Rows: price, display size, camera, battery, and 'Best for' summary (11px #666). Current Galaxy S26 Ultra column highlighted with blue (#F0F6FF) background. 'Help me choose' button at panel bottom (44px, #2189FF, full-width). Phone product page dimmed with semi-transparent overlay in background.",
  ...
  },
  {
    ...
    categorySlug: "cta",
    mockupPrompt:
      "UI mockup screenshot, Samsung AU product page bottom portion on mobile (393×852). Sticky bar at bottom (56px height, dark #1A1A2E background) with blue 'Add to Cart — $1,949' button (#2189FF, 48px height, 24px rounded corners, white 15px bold text). Small accessory card (80px height, white, 1px #E8E8E8 border, 8px rounded top corners) above the sticky bar showing Galaxy Buds3 Pro with 40px thumbnail left, text 'Complete your setup — Add Buds3 Pro and save $50' in 13px #333, and '$2,248 (save $50)' in 14px bold green (#00C853) right. Two buttons below: 'Add phone only' (white outlined, 1px #CCC, 32px) and 'Add both — Save $50' (filled blue #2189FF, 32px). Product page dimmed behind dark overlay. Clean Samsung mobile design.",
  ...
  },
  {
    ...
    categorySlug: "trust",
    mockupPrompt:
      "UI mockup screenshot, Samsung AU product buy page between color/storage selector and hubble-price-bar on desktop. Horizontal trust bar spanning full content width (960px) with 1px border (#E8E8E8) separating from configurator above, 56px height. 5 icons in equally-spaced row: shield (Australian Warranty), truck (Free Delivery), return arrow (14-Day Returns), lock (Secure Checkout), Afterpay logo (16px). Each icon in blue-gray (#6B7B8D) at 20px with label below in 11px #757575 Samsung One, separated by 1px dividers (#EEE, 24px). Background: very light gray (#FAFAFA). Below bar, hubble-price-bar with price '$1,949' in 20px bold #111 and blue 'Add to Cart' button (#2189FF, 44px height). Clean design with generous whitespace. Desktop viewport 1440×900.",
  ...
  },
  {
    ...
    mockupPrompt:
      "UI mockup screenshot, Samsung AU homepage on desktop (1440×900). Hero banner (hd08-hero-kv-home) with dark gradient, Galaxy S26 Ultra hero image right, CTA headline left. Below, 'Recently Viewed' section using co75-explore-carousel pattern with heading in 22px Samsung One bold #111 and 'Clear history' link 12px #2189FF. Horizontal carousel with arrow navigation (32px white circles). 4 product cards (220px each, 12px gap, 1px #EEE border, 8px rounded): Galaxy S26 Ultra (silver, $1,949), Galaxy Watch7 (green, $549), 65-inch Neo QLED TV ($2,799), Galaxy Buds3 Pro (white, $349). Each card: product image (160px), name 14px #333, price 16px bold #111, 'Continue browsing' link 12px #2189FF below. White background, blue (#2189FF) accent. Desktop viewport 1440×900.",
  ...
  },
  {
    ...
    mockupPrompt:
      "UI mockup screenshot, Samsung AU cart page (shop.samsung.com/au/cart) on desktop (1440×900). Below 'Shopping Cart' header in 24px bold #111: free shipping progress bar (full content width, 8px height, 16px rounded ends, light gray #E8E8E8 track) at 90% fill with green gradient (#00C853 to #69F0AE). Above bar: 'You're $20 away from FREE delivery! 🚚' in 15px #333. Below bar: 3 product suggestion cards in row — Galaxy S26 Ultra screen protector ($29), 25W USB-C charger ($35), clear standing case ($39). Each card: 180px wide, 1px #EEE border, 8px rounded, '+' quick-add button (24px circle, #2189FF outline). Below suggestions: cart line items with Galaxy S26 Ultra ($1,949), product thumbnail, quantity selector, and remove icon. Clean white background with 24px content padding.",
